...
if(whichConfig == 1) {
    ...
    await $`awk '{gsub(/NEXT_PUBLIC_REST_API_ENDPOINT=.+"$/,"NEXT_PUBLIC_REST_API_ENDPOINT=\\"https://${domainName}/backend\\""); print $0}' ./shop/.env.template > ./shop/.env`;
    ...
    await $`awk '{gsub(/NEXT_PUBLIC_REST_API_ENDPOINT=.+"$/,"NEXT_PUBLIC_REST_API_ENDPOINT=\\"https://${domainName}/backend\\""); print $0}' ./admin/rest/.env.template > ./admin/rest/.env`;
    ...
    await $`cp ./shop/.env.template ./shop/.env`
    await $`chmod 777 ./shop/.env`
    await $`awk '{gsub(/NEXT_PUBLIC_GRAPHQL_API_ENDPOINT=.+"$/,"NEXT_PUBLIC_GRAPHQL_API_ENDPOINT=\\"https://${domainName}/backend/graphql\\""); print $0}' ./shop/.env.template > ./shop/.env`;
    ...
    await $`awk '{gsub(/NEXT_PUBLIC_GRAPHQL_API_ENDPOINT=.+"$/,"NEXT_PUBLIC_GRAPHQL_API_ENDPOINT=\\"https://${domainName}/backend/graphql\\""); print $0}' ./admin/graphql/.env.template > ./admin/graphql/.env`;
